...
monkeypatched with in-memory fakes, so nothing here touches Firestore.
...
def _install_fake_sessions(monkeypatch, sessions):
    ...
    monkeypatch.setattr(FirebaseService, 'get_db', classmethod(lambda cls: fake_db))

    captured = []

    def fake_update_session(cls, session_id, data):
        captured.append((session_id, data))
        return None

    monkeypatch.setattr(FirebaseService, 'update_session', classmethod(fake_update_session))
    return captured
...
def _install_check_in_fakes(monkeypatch, session):
    ...
    monkeypatch.setattr(FirebaseService, 'get_db', classmethod(lambda cls: _FakeDb()))
    monkeypatch.setattr(FirebaseService, 'get_session', classmethod(lambda cls, session_id: dict(session)))
    return captured
